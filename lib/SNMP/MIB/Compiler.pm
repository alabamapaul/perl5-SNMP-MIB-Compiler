...
CHAR: while ($c ne '' && $c !~ m/^[A-Za-z0-9:=,\{\}<.\(\)\[\]\'\">|]$/o)
  {
    ...
    return 0 if $c eq '';
    if ($c eq '-')
    {             # The first char of a "comment" (See Section 8.6)
      $c = $s->getc;
      if ($c eq '')
      {           # a single hyphen followed by EOF
        $s->ungetc;    # keep EOF for the next yylex call
        $c = '-';
        last CHAR;
      }
      if ($c eq '-')
      {                # it is a real "comment" marker
        while (1)
        {
          1 while ($c = $s->getc) ne ''
            && $c ne '-'
      ...
      {                               # it is NOT a comment but a single hyphen.
        $s->ungetc;
        $c = '-';
        last CHAR;
      }
  ...
  if ($c eq '\'')
  {    # it can be a cstring or a hstring.
    $val = $c;
  ...
  if ($c =~ m/[a-zA-Z]/o)
  {
    ...
    while (1)
    {
      ...
      if ($c !~ m/[A-Za-z0-9]/o)
      {
        ...
        return ($MINUSINFINITY, $val) if $val eq 'MINUS-INFINITY';
        return $self->assert(
          MIBERROR,       $self->{'filename'},
          $s->{'lineno'}, "'$val' unrecognized"
        );
      }
    }
  }
...
sub unget_token
{
  ...
  {
    push @{$self->{'token_list'}},
      [$self->{'current_token'}, $self->{'current_value'}, $self->{'lineno'}];
    ...
    if (defined $self->{'nodes'}{$$list[0]})
    {
        ...
        $#{$self->{'nodes'}{$$list[0]}{'oid'}})
      {
        ...
        for (@{$self->{'nodes'}{$$list[0]}{'OID'}})
        {
          $i++;
          $eq = 0, last
            unless ${$self->{'nodes'}{$$list[0]}{'OID'}}[$i] eq
            ${$self->{'nodes'}{$$list[0]}{'oid'}}[$i];
        }
        unless ($eq)
        {
  ...
  elsif ($token && $value eq '{')
  {
    ...
    while ($value ne '}')
    {
      ...
      if ($token == $IDENTIFIER)
      {
        ...
        return $self->assert(
          MIBERROR,          $self->{'filename'},
          $self->{'lineno'}, "must have a subtype"
        ) unless defined $res;
        $$list{$res} = $value;
      }
      else
...
sub parse_type
{
  ...
  elsif ($value eq 'TEXTUAL-CONVENTION')
  {      # textual convention
    ...
    while ($value ne 'MODULE' && $token != $ASSIGNMENT)
    {
      ...
      elsif ($value eq 'OBJECT')
      {
        ...
        if ($value eq 'MIN-ACCESS')
        {
          (($token, $value) = $self->get_token()) || return;
          if (
            $value =~ m/^(read-(only|write|create)|not-accessible|
    ...
    while (defined $token && $value ne 'SUPPORTS' && $token != $ASSIGNMENT)
    {
      ...
      while ($value eq 'VARIATION')
      {
        ...
        if ($value eq 'ACCESS')
        {
          (($token, $value) = $self->get_token()) || return;
          if (
            $value =~ m/^(not-implemented|accessible-for-notify|
...
sub parse_objecttype
{
  ...
  if ($value eq 'ACCESS' || $value eq 'MAX-ACCESS')
  {
    if ($value eq 'MAX-ACCESS')
    {
      ...
      (($token, $value) = $self->get_token()) || return;
      if (
        $value =~ m/^(read-(only|write)|not-accessible|
  ...
  {
    return $self->assert(MIBERROR, $self->{'filename'}, $self->{'lineno'},
      "Can't define both 'INDEX' and 'AUGMENTS'")
...
sub parse_imports
{
  ...
  while (defined $token && $value ne ';')
  {
    ...
    {
      return $self->assert(MIBERROR, $self->{'filename'}, $self->{'lineno'},
        "two values must be separated by a comma")
        if $elem;
      push @$list, $value;
      $elem = 1;
    }
...
sub tree
{
  ...
  for my $n (sort { $a <=> $b } keys %{$self->{'tree'}{$node}})
  {
    ...
    $access .= " " if $access;
    my $type = "";
    $type = $self->{'nodes'}{$new}{'syntax'}{'type'}
    ...
    if ($type)
    {
      ...
      $type = sprintf "%-8.8s ",
        defined $$treetypes{$type} ? $$treetypes{$type} : $type;
      $type = "" if $type =~ m/^\s+$/o;
    }
    $s .= $access . $type . $new . '(' . $n . ")\n";
